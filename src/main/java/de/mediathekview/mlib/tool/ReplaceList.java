/*
 * MediathekView Copyright (C) 2014 W. Xaver W.Xaver[at]googlemail.com
...
public final class ReplaceList {
  ...
  public static boolean check() {
    for (int i = 0; i < list.size(); ++i) {
      final String[] is = list.get(i);
      for (int k = i + 1; k < list.size(); ++k) {
        final String[] ks = list.get(k);
  ...
  public static String replace(String strCheck, final boolean pfad) {
    final Iterator<String[]> it = list.iterator();
    while (it.hasNext()) {
      final String[] strReplace = it.next();
  ...
  public static int up(final int idx, final boolean up) {
    final String[] replace = list.remove(idx);
    int neu = idx;
    if (up) {
      if (neu > 0) {
        --neu;

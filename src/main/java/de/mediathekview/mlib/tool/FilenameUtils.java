...
public class FilenameUtils {
  ...
  /**
   * Valid characters for Windows in file names: Based on
  ...
  public static String checkDateiname(final String name, final boolean isPath) {
    ...
    if (SystemUtils.IS_OS_WINDOWS) {
      ...
      if (isPath) {
        if (ret.length() > 1 && ret.charAt(1) == ':') {
          ...
          isWindowsPath = true;
          ret = ret.replaceFirst(":", ""); // muss zum Schluss wieder rein, kann aber so nicht
                                           // ersetzt werden
        }
      }
    } else {
    ...
    if (isPath && ret.contains(File.separator)) {
      ...
      for (final String s : sa) {
        if (!s.isEmpty()) {
          str += File.separator + convertToNativeEncoding(s, false); // sind ja nur noch die
                                                                     // Ordnernamen
        }
      }
  ...
  public static String removeIllegalCharacters(final String input, final boolean isPath) {
    ...
    switch (Functions.getOs()) {
      ...
      case LINUX:
        ...
        ret = removeStartingDots(ret);
        ret = ret.replaceAll(
            isPath ? REGEXP_ILLEGAL_CHARACTERS_OTHERS_PATH : REGEXP_ILLEGAL_CHARACTERS_OTHERS, "_");
        break;
      ...
      case WIN32:
        ...
        ret = removeWindowsTrailingDots(ret);
        ret = ret.replaceAll(
            isPath ? REGEXP_ILLEGAL_CHARACTERS_WINDOWS_PATH : REGEXP_ILLEGAL_CHARACTERS_WINDOWS,
            "_");
        break;
  ...
  private static String cleanUnicode(final String ret) {
    String r = "";
    char c;
   ...
   */
  private static String convertToASCIIEncoding(final String fileName, final boolean isPath) {
    String ret = fileName;
    ...
    try {
      ...
      charsetEncoder.onMalformedInput(CodingErrorAction.REPLACE); // otherwise breaks on first
                                                                  // unconvertable char
   ...
   */
  private static String convertToNativeEncoding(final String fileName, final boolean isPath) {
    String ret = fileName;

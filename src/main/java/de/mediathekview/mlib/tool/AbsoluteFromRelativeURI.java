 ...
 *
 * <p>
 * This is a generic utility for resolving URIs, other than the fact that it's declared to throw
...
public class AbsoluteFromRelativeURI {
   ...
   *
   * <p>
   * The relative URI is a local filesystem path. The path can be absolute or relative. If it is a
   * relative path, it is resolved relative to the system property "user.dir" if it is available; if
   * not (i.e. in an Applet perhaps which throws SecurityException) then we just return the relative
   * path. The space and backslash characters are also replaced to generate a good absolute URI.
   * </p>
   *
  ...
  /**
   * Replace spaces with "%20" and backslashes with forward slashes in the input string to generate

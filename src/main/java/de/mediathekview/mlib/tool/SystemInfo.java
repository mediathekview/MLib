...
public class SystemInfo {
  ...
  private OperatingSystemType readOs() {
    ...
    if (com.jidesoft.utils.SystemInfo.isLinux()
  ...
  private void readRamInfo() {
    raminfo = new Ram(Runtime.getRuntime().totalMemory(), Runtime.getRuntime().maxMemory(),
  ...
  private void readJarPath() {
    try {
      String ermittelterPfad =
          SystemInfo.class.getProtectionDomain().getCodeSource().getLocation().getPath();
  ...
  private void readJavaVersion() {
    javaversion =
        new JavaVersion(System.getProperty("java.vendor"), System.getProperty("java.vm.name"),

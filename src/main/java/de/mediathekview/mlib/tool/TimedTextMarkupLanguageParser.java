/*
 * TimedTextMarkupLanguageParser Copyright (C) 2016 CrystalPalace crystalpalace1977@googlemail.com
 *
...
public class TimedTextMarkupLanguageParser {
  ...
  public boolean parse(final Path ttmlFilePath) {
    ...
    try {
      ...
      // Log.errorLog(912036478, ex, new String[]{ex.getLocalizedMessage(), "File: " +
      // ttmlFilePath});
  ...
  public void toSrt(final Path srtFile) {
    try (FileOutputStream fos = new FileOutputStream(srtFile.toFile());
      ...
      long counter = 1;
      for (final Subtitle title : subtitleList) {
        writer.println(counter);
        writer.println(srtFormat.format(title.begin) + " --> " + srtFormat.format(title.end));
        for (final StyledString entry : title.listOfStrings) {
  ...
  private Date parseFlash(final String tStamp) throws ParseException {
    Date da;
    if (tStamp.contains(":")) {

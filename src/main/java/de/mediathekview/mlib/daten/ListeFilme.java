...
public class ListeFilme extends ArrayList<Film> {
  ...
  public static final String[] COLUMN_NAMES =
      {FILMLISTE_DATUM, FILMLISTE_DATUM_GMT, FILMLISTE_VERSION, FILMLISTE_PROGRAMM, FILMLISTE_ID};
  ...
  @Override
  public boolean add(final Film aFilm) {
    indexes.add(String.valueOf(aFilm.hashCode()));
    return super.add(aFilm);
  }

  public synchronized boolean addFilmVomSender(final Film film) {
    // Filme die beim Sender gesucht wurden (und nur die) hier eintragen, nur für die
    // MediathekReader!!
    // ist: "Sender-Thema-URL" schon vorhanden, wird sie verworfen

    return !contains(film) && addInit(film);
  }
   ...
   */
  public synchronized void deleteAllFilms(final String sender) {
   ...
   */
  @Deprecated
  public String getFileSizeUrl(final String url) {
    // FIXME bring to DatenFilm and reduce calculation
    String res;

    final Optional<Film> opt =
        parallelStream().filter(f -> f.getUrl(Resolution.NORMAL).equals(url)).findAny();
    if (opt.isPresent()) {
      final Film film = opt.get();
  ...
  public synchronized Film getFilmByUrl(final String url) {
    final Optional<Film> opt = parallelStream()
        .filter(f -> f.getUrl(Resolution.NORMAL).toString().equalsIgnoreCase(url)).findAny();
    return opt.orElse(null);
  }

  public synchronized Film getFilmByUrl_klein_hoch_hd(final String url) {
    // Problem wegen gleicher URLs
    // wird versucht, einen Film mit einer kleinen/Hoher/HD-URL zu finden
    Film ret = null;
    for (final Film f : this) {
  ...
  public synchronized void getThema(final String sender, final LinkedList<String> liste) {
    stream().filter(film -> film.getSenderName().equals(sender))
        .filter(film -> !liste.contains(film.getThema()))
        .forEach(film -> liste.add(film.getThema()));
  }

  public synchronized boolean importFilmliste(final Film film) {
   ...
   */
  public boolean isOlderThan(final int sekunden) {
  ...
  public synchronized boolean isTooOldForDiff() {
    ...
    try {
      final String dateMaxDiff_str = new SimpleDateFormat("yyyy.MM.dd__").format(new Date())
          + Const.TIME_MAX_AGE_FOR_DIFF + ":00:00";
  ...
  public synchronized ListeFilme neueFilme(final ListeFilme orgListe) {
    ...
    final ListeFilme ret = new ListeFilme();
    final HashSet<String> hashSet = new HashSet<>(orgListe.size() + 1, 1);

    for (final Film film : orgListe) {
  ...
  public synchronized void setMeta(final ListeFilme listeFilme) {
  ...
  public synchronized void updateListe(final ListeFilme listeEinsortieren,
      final boolean index /* Vergleich über Index, sonst nur URL */, final boolean ersetzen) {
    ...
    if (ersetzen) {
      listeEinsortieren.forEach((final Film aktuellerFilm) -> addHash(aktuellerFilm, hash, index));
  ...
  public synchronized void writeMetaData() {
    ...
    metaDaten[ListeFilme.FILMLISTE_ID_NR] =
        createChecksum(metaDaten[ListeFilme.FILMLISTE_DATUM_GMT_NR]);
    metaDaten[ListeFilme.FILMLISTE_VERSION_NR] = Const.VERSION_FILMLISTE;
    metaDaten[ListeFilme.FILMLISTE_PRGRAMM_NR] =
        Const.PROGRAMMNAME + Functions.getProgVersionString(); // + " - Compiled: " +
                                                               // Functions.getCompileDate();
  }

  private void addHash(final Film f, final HashSet<String> hash, final boolean aCheckWithIndex) {
    if (aCheckWithIndex) {
  ...
  private boolean addInit(final Film film) {
    return add(film);
  }
   ...
   */
  private String createChecksum(final String input) {
  ...
  private String getJetzt_ddMMyyyy_HHmm_gmt() {
    final SimpleDateFormat formatter = new SimpleDateFormat(DATUM_ZEIT_FORMAT);

...
public class FilmToFakeJsonConverter {
	...
	private static final String[] COLUMNNAMES = new String[] { "Sender", "Thema", "Titel", "Datum", "Zeit", "Dauer",
			"Größe [MB]", "Beschreibung", "Url", "Website", "Url Untertitel", "Url RTMP", "Url Klein", "Url RTMP Klein",
	...
	private void resourceToFakeJson(final StringBuilder fakeJsonBuilder, final AbstractMediaResource<?> aMediaResource,
	...
	private void appendAudioDescriptionEntry(final AbstractMediaResource<?> aMediaResource, final String aTitle,
			final StringBuilder fakeJsonBuilder, final boolean aIsLastFilm) {
		String url;
		String urlSmall = "";
		String urlHd = "";
		...
		if (aMediaResource instanceof Film) {
			...
			if (filmUrl != null) {
				...
				appendMediaResource(fakeJsonBuilder, aMediaResource, titleAudioDescription, urlSmall, url, urlHd,
						aIsLastFilm);
			}
		}
	}

	private void appendSignLanguageEntry(final AbstractMediaResource<?> aMediaResource, final String aTitle,
			final StringBuilder fakeJsonBuilder, final boolean aIsLastFilm) {
		String url;
		String urlSmall = "";
		String urlHd = "";
		...
		if (aMediaResource instanceof Film) {
			...
			if (filmUrl != null) {
				...
				appendMediaResource(fakeJsonBuilder, aMediaResource, titleSignLanguage, urlSmall, url, urlHd,
						aIsLastFilm);
			}
		}
	}

	private void appendMediaResource(final StringBuilder fakeJsonBuilder, final AbstractMediaResource<?> aMediaResource,
			final String aTitle, String aUrlSmall, final String aUrlNormal, String aUrlHd, final boolean aIsLastFilm) {
		...
		fakeJsonBuilder.append(String.format(OUTPUT_PATTERN, sender, gson.toJson(thema), gson.toJson(aTitle),
				aMediaResource.getTime() == null ? "" : DATE_FORMATTER.format(aMediaResource.getTime().toLocalDate()),
				aMediaResource.getTime() == null ? "" : TIME_FORMATTER.format(aMediaResource.getTime().toLocalTime()),
				aMediaResource instanceof Podcast ? durationToString(((Podcast) aMediaResource).getDuration()) : "",
				aMediaResource instanceof Podcast
						? ((Podcast) aMediaResource).getFileSize(getDefaultResolution(aMediaResource))
						: "",
				gson.toJson(aMediaResource.getBeschreibung()), aUrlNormal, website, getSubtitles(aMediaResource), "",
				aUrlSmall, "", aUrlHd, "", convertDateTimeToLong(aMediaResource.getTime()), "", // History

...
public class ListeFilmlistenUrls extends LinkedList<DatenFilmlisteUrl> {
  ...
  public boolean addWithCheck(final DatenFilmlisteUrl filmliste) {
    for (final DatenFilmlisteUrl datenUrlFilmliste : this) {
      if (datenUrlFilmliste.arr[DatenFilmlisteUrl.FILM_UPDATE_SERVER_URL_NR]
          .equals(filmliste.arr[DatenFilmlisteUrl.FILM_UPDATE_SERVER_URL_NR])) {
        return false;
      }
    }
  ...
  public DatenFilmlisteUrl getDatenUrlFilmliste(final String url) {
  ...
  public String getRand(final ArrayList<String> bereitsGebraucht) {
    ...
    final LinkedList<DatenFilmlisteUrl> listePrio = new LinkedList<>();
    // nach prio gewichten
    for (final DatenFilmlisteUrl datenFilmlisteUrl : this) {
      if (bereitsGebraucht != null) {
        if (bereitsGebraucht
            .contains(datenFilmlisteUrl.arr[DatenFilmlisteUrl.FILM_UPDATE_SERVER_URL_NR])) {
          // wurde schon versucht
          continue;
        }
        if (datenFilmlisteUrl.arr[DatenFilmlisteUrl.FILM_UPDATE_SERVER_PRIO_NR]
            .equals(DatenFilmlisteUrl.FILM_UPDATE_SERVER_PRIO_1)) {
    ...
    if (!listePrio.isEmpty()) {
      final int nr = new Random().nextInt(listePrio.size());
      datenFilmlisteUrl = listePrio.get(nr);
    } else {
      // dann wird irgendeine Versucht
      final int nr = new Random().nextInt(size());
      datenFilmlisteUrl = get(nr);
    }
  ...
  public String[][] getTableObjectData() {
    ...
    String[][] object;
    final Iterator<DatenFilmlisteUrl> iterator = iterator();
    object = new String[size()][DatenFilmlisteUrl.FILM_UPDATE_SERVER_MAX_ELEM];
    int i = 0;
    while (iterator.hasNext()) {
      filmUpdate = iterator.next();
      System.arraycopy(filmUpdate.arr, 0, object[i], 0,
          DatenFilmlisteUrl.FILM_UPDATE_SERVER_MAX_ELEM);
      object[i][DatenFilmlisteUrl.FILM_UPDATE_SERVER_DATUM_NR] = filmUpdate.getDateStr(); // lokale
                                                                                          // Zeit
                                                                                          // anzeigen
      object[i][DatenFilmlisteUrl.FILM_UPDATE_SERVER_ZEIT_NR] = filmUpdate.getTimeStr(); // lokale
                                                                                         // Zeit
                                                                                         // anzeigen
      ++i;
    }
  ...
  // if (bereitsGebraucht != null) {
  // if
  // (bereitsGebraucht.contains(datenUrlFilmliste.arr[DatenFilmlisteUrl.FILM_UPDATE_SERVER_URL_NR]))
  // {
  // // wurde schon versucht
  // continue;
  // }
  ...
  // datenUrlFilmliste = it.next();
  // if
  // (datenUrlFilmliste.arr[DatenFilmlisteUrl.FILM_UPDATE_SERVER_PRIO_NR].equals(DatenFilmlisteUrl.FILM_UPDATE_SERVER_PRIO_1))
  // {

...
public class FilmlistenSuchen {
  ...
  private final int UPDATE_LISTE_MAX = 10; // die Downloadliste für die Filmlisten nur jeden 10.
                                           // Programmstart aktualisieren

  public void getDownloadUrlsFilmlisten(final String dateiUrl,
      final ListeFilmlistenUrls listeFilmlistenUrls, final String userAgent, final String art) {
    ...
    try {
      ...
      if (Functions.istUrl(dateiUrl)) {
        ...
        final File f = new File(dateiUrl);
        if (!f.exists()) {
          return;
        }
      ...
      while (parser.hasNext()) {
        ...
        if (event == XMLStreamConstants.START_ELEMENT) {
          final String parsername = parser.getLocalName();
  ...
  public String suchenAkt(final ArrayList<String> bereitsVersucht) {
    ...
    if (listeFilmlistenUrls_akt.isEmpty()) {
      ...
      final int nr = new Random().nextInt(UPDATE_LISTE_MAX);
      if (nr == 0) {
        updateURLsFilmlisten(true);
      }
    }
  ...
  public String suchenDiff(final ArrayList<String> bereitsVersucht) {
    // passende URL zum Laden der Filmliste suchen
    String retUrl;
  ...
  public void updateURLsFilmlisten(final boolean updateFullList) {
    final ListeFilmlistenUrls tmp = new ListeFilmlistenUrls();
    if (updateFullList) {
      getDownloadUrlsFilmlisten(Const.ADRESSE_FILMLISTEN_SERVER_AKT, tmp, Config.getUserAgent(),
          DatenFilmlisteUrl.SERVER_ART_AKT);
      if (!tmp.isEmpty()) {
      ...
      getDownloadUrlsFilmlisten(Const.ADRESSE_FILMLISTEN_SERVER_DIFF, tmp, Config.getUserAgent(),
          DatenFilmlisteUrl.SERVER_ART_DIFF);
      if (!tmp.isEmpty()) {
    ...
    if (tmp.isEmpty()) {
      Log.errorLog(491203216, new String[] {"Es ist ein Fehler aufgetreten!",
          "Es konnten keine Updateserver zum aktualisieren der Filme", "gefunden werden."});
    }
  }
  ...
  private void insertDefaultActiveServers() {
    listeFilmlistenUrls_akt.add(new DatenFilmlisteUrl("http://m.picn.de/f/Filmliste-akt.xz",
        DatenFilmlisteUrl.SERVER_ART_AKT));
    listeFilmlistenUrls_akt.add(new DatenFilmlisteUrl("http://m1.picn.de/f/Filmliste-akt.xz",
        DatenFilmlisteUrl.SERVER_ART_AKT));
    listeFilmlistenUrls_akt.add(new DatenFilmlisteUrl("http://m2.picn.de/f/Filmliste-akt.xz",
        DatenFilmlisteUrl.SERVER_ART_AKT));
    listeFilmlistenUrls_akt.add(new DatenFilmlisteUrl(
        "http://download10.onlinetvrecorder.com/mediathekview/Filmliste-akt.xz",
        DatenFilmlisteUrl.SERVER_ART_AKT));
    listeFilmlistenUrls_akt.add(new DatenFilmlisteUrl(
        "http://mediathekview.jankal.me/Filmliste-akt.xz", DatenFilmlisteUrl.SERVER_ART_AKT));
    listeFilmlistenUrls_akt.add(new DatenFilmlisteUrl(
        "http://verteiler1.mediathekview.de/Filmliste-akt.xz", DatenFilmlisteUrl.SERVER_ART_AKT));
    listeFilmlistenUrls_akt.add(new DatenFilmlisteUrl(
        "http://verteiler2.mediathekview.de/Filmliste-akt.xz", DatenFilmlisteUrl.SERVER_ART_AKT));
    listeFilmlistenUrls_akt.add(new DatenFilmlisteUrl(
        "http://verteiler3.mediathekview.de/Filmliste-akt.xz", DatenFilmlisteUrl.SERVER_ART_AKT));
  }
  ...
  private void insertDefaultDifferentialListServers() {
    listeFilmlistenUrls_diff.add(new DatenFilmlisteUrl("http://m.picn.de/f/Filmliste-diff.xz",
        DatenFilmlisteUrl.SERVER_ART_DIFF));
    listeFilmlistenUrls_diff.add(new DatenFilmlisteUrl("http://m1.picn.de/f/Filmliste-diff.xz",
        DatenFilmlisteUrl.SERVER_ART_DIFF));
    listeFilmlistenUrls_diff.add(new DatenFilmlisteUrl("http://m2.picn.de/f/Filmliste-diff.xz",
        DatenFilmlisteUrl.SERVER_ART_DIFF));
    listeFilmlistenUrls_diff.add(new DatenFilmlisteUrl(
        "http://download10.onlinetvrecorder.com/mediathekview/Filmliste-diff.xz",
        DatenFilmlisteUrl.SERVER_ART_DIFF));
    listeFilmlistenUrls_diff.add(new DatenFilmlisteUrl(
        "http://mediathekview.jankal.me/Filmliste-diff.xz", DatenFilmlisteUrl.SERVER_ART_DIFF));
    listeFilmlistenUrls_diff.add(new DatenFilmlisteUrl(
        "http://verteiler1.mediathekview.de/Filmliste-diff.xz", DatenFilmlisteUrl.SERVER_ART_DIFF));
    listeFilmlistenUrls_diff.add(new DatenFilmlisteUrl(
        "http://verteiler2.mediathekview.de/Filmliste-diff.xz", DatenFilmlisteUrl.SERVER_ART_DIFF));
    listeFilmlistenUrls_diff.add(new DatenFilmlisteUrl(
        "http://verteiler3.mediathekview.de/Filmliste-diff.xz", DatenFilmlisteUrl.SERVER_ART_DIFF));
  }
   ...
   * @param art
   */
  private void parseServerEntry(final XMLStreamReader parser,
      final ListeFilmlistenUrls listeFilmlistenUrls, final String art) {
    String serverUrl = "";
    String prio = "";
    int event;
    try {
      while (parser.hasNext()) {
        ...
        if (event == XMLStreamConstants.END_ELEMENT) {
          ...
          if (parser.getLocalName().equals("Server")) {
            if (!serverUrl.equals("")) {
              // public DatenFilmUpdate(String url, String prio, String zeit, String datum, String
              // anzahl) {
              if (prio.equals("")) {

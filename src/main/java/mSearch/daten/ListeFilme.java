...
public class ListeFilme extends ArrayList<DatenFilm> {
    ...
    protected final static String DATUM_ZEIT_FORMAT = "dd.MM.yyyy, HH:mm";
...
//    }
    ...
    protected boolean addInit(DatenFilm film) {
        film.init();
        return add(film);
    }
    ...
    public synchronized DatenFilm getFilmByUrl_klein_hoch_hd(String url) {
        ...
        for (DatenFilm f : this) {
            if (f.arr[DatenFilm.FILM_URL].equals(url)) {
                ret = f;
                break;
            } else if (f.getUrlFuerAufloesung(Aufloesungen.HD.getDescription()).equals(url)) {
                ret = f;
                break;
            } else if (f.getUrlFuerAufloesung(Aufloesungen.KLEIN.getDescription()).equals(url)) {
                ret = f;
                break;
            }
        }

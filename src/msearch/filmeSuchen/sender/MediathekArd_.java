 ...
 * @author
 */
public class MediathekArd_ extends MediathekReader implements Runnable {
     ...
     * @param startPrio
     */
    public MediathekArd_(MSFilmeSuchen ssearch, int startPrio) {
        super(ssearch, /* name */ SENDER, /* threads */ 10, /* urlWarten */ 500, startPrio);
    }

    @Override
    void addToList() {
        final String ADRESSE = "http://m.ardmediathek.de/Sendungen-A-Z?pageId=13932746";
        final String MUSTER_URL = "<a href=\"/Sendungen-mit-";
        listeThemen.clear();
        ...
        while ((pos = seite.indexOf(MUSTER_URL, pos)) != -1) {
            try {
                ...
                url = "http://m.ardmediathek.de/Sendungen-mit-" + url;
                url = url.replace("&amp;", "&");
                sendungenSuchen(url);
            } catch (Exception ex) {
...
//////            for (int t = 0; t < 1; ++t) {
    ...
    private void sendungenSuchen(String strUrlFeed) {
        final String MUSTER = "<li data-icon=\"";
        ...
        while (!MSConfig.getStop() && (pos = seite_1.indexOf(MUSTER, pos)) != -1) {
            try {
                pos += MUSTER.length();
                url = seite_1.extract("<a href=\"/", "\"", pos);
                if (url.equals("")) {
                    continue;
                }
                url = "http://m.ardmediathek.de/" + url;
                url = url.replace("&amp;", "&");
                thema = seite_1.extract("<h3>", "<", pos);
                if (thema.isEmpty()) {
...
//        private final MSStringBuilder seite4 = new MSStringBuilder(MSConst.STRING_BUFFER_START_BUFFER);
        ...
        private void filmSuchen1(String strUrlFeed, String thema) {
            final String MUSTER_1 = "<h1>";
            final String MUSTER_2 = "<li data-icon=";
            final String MUSTER_START = "<div  class=\"pagingList\">";
            ...
            int pos1 = 0, pos2 = 0, pos3 = 0, posDate = 0;
            String url, datum, zeit = "", titel, dauer, urlSendung;
            long d = 0;
            int count = 0;
            if ((pos1 = seite2.indexOf(MUSTER_START)) != -1) {
                pos1 += MUSTER_START.length();
            } else {
                return;
            }
            while (!MSConfig.getStop() && (pos1 = seite2.indexOf(MUSTER_1, pos1)) != -1) {
                datum = "";
                zeit = "";
                ++count;
                ...
                pos1 += MUSTER_1.length();
                pos2 = pos1;
                pos3 = seite2.indexOf(MUSTER_1, pos1);
                if ((posDate = seite2.indexOf("<", pos1)) != -1) {
                    datum = seite2.substring(pos1, posDate);
                ...
                while (!MSConfig.getStop() && (pos2 = seite2.indexOf(MUSTER_2, pos2)) != -1) {
                    pos2 += MUSTER_2.length();
                    d = 0;
                    if (pos2 > pos3 && pos3 != -1) {
                        break;
                    }
                    url = seite2.extract("<a href=\"", "\"", pos2);
                    if (url.equals("")) {
                        continue;
                    }
                    url = "http://m.ardmediathek.de" + url;
                    url = url.replace("&amp;", "&");
                    titel = seite2.extract("<h2>", "<", pos2);
                    dauer = seite2.extract("| ", "<", pos2);
                    ...
                    filmSuchen2(url, thema, titel, d, datum, zeit);
                }

            }
        }

        private void filmSuchen2(String urlFilm, String thema, String titel, long dauer, String datum, String zeit) {
            try {
                meldung(urlFilm);
                ...
                if (!url.isEmpty() && url.startsWith("http")) {
                    DatenFilm f = new DatenFilm(nameSenderMReader, thema, urlFilm, titel, url, ""/*urlRtmp*/, datum, zeit, dauer, beschreibung,
